...
escaped_dollar = L("$$").set_parse_action(lambda: "$")
lone_dollar = L("$")
...
delimited_expr = "{" + Group(brace_balanced_expr).set_results_name("x") + "}"
delimited_expr.set_parse_action(
    lambda ts: "".join(ts.x),
...
interpolation = (
    "$" + (simple_expr | delimited_expr).set_results_name("expr")
).set_parse_action(lambda ts: ts.expr)

escaped_interpolation = (
    "$!" + (simple_expr | delimited_expr).set_results_name("expr")
).set_parse_action(lambda ts: ts.expr[0].noescape())
...
text_with_interpolations = ZeroOrMore(
    ...
    | escaped_dollar
    | lone_dollar
).set_parse_action(lambda ts: list(group_strings(ts)))

interpolation_parser = text_with_interpolations()
interpolation_parser.leave_whitespace()
interpolation_parser.parse_with_tabs()
